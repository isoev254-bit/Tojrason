// Tojrason/frontend/courier/src/api/axios.config.ts
/*
...
axiosInstance.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    const token = localStorage.getItem('courier_accessToken');
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        localStorage.setItem('courier_accessToken', accessToken);
        localStorage.setItem('courier_refreshToken', newRefreshToken);
...
export const setAuthToken = (token: string | null) => {
  if (token) {
    localStorage.setItem('courier_accessToken', token);
...
export const getAuthToken = (): string | null => {
  return localStorage.getItem('courier_accessToken');
};
