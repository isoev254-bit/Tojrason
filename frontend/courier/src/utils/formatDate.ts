// Tojrason/frontend/courier/src/utils/formatDate.ts
/*
...
/**
 * Форматкунии вақт барои намоиш дар огоҳиҳои фармоиш
 * @param date - Санаи фармоиш
 * @returns Сатри форматшуда барои огоҳӣ
 */
export const formatOrderTime = (date: string | number | Date | null | undefined): string => {
  const parsedDate = parseDate(date);
  if (!parsedDate) return '';
  
  if (isToday(parsedDate)) {
    return `Имрӯз, ${formatTime(parsedDate)}`;
  }
  
  if (isYesterday(parsedDate)) {
    return `Дирӯз, ${formatTime(parsedDate)}`;
  }
