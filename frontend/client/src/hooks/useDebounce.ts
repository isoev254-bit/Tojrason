// Tojrason/frontend/client/src/hooks/useDebounce.ts
...
export const useDebouncedCallback = <T extends (...args: any[]) => any>(
  ...
  // Навсозии callback дар ref
  useEffect(() => {
    callbackRef.current = callback;
  }, [callback]);

  // Тоза кардани timeout ҳангоми unmount
  useEffect(() => {
    return () => {
  ...
  const debouncedCallback = useCallback((...args: Parameters<T>) => {
    // Агар timeout мавҷуд бошад, онро тоза мекунем
    if (timeoutRef.current) {
      clearTimeout(timeoutRef.current);
    }

    // Timeout-и нав месозем
...
export const useDebounce = <T>(value: T, delay: number = 500): T => {
  ...
  useEffect(() => {
    // Timeout барои навсозии қимат
    const timeout = setTimeout(() => {
      setDebouncedValue(value);
    }, delay);

    // Тоза кардани timeout агар value ё delay тағйир ёбад
    return () => {
...
export const useThrottle = <T extends (...args: any[]) => any>(
  ...
  const throttledCallback = useCallback((...args: Parameters<T>) => {
    ...
    if (lastRunRef.current === 0) {
      // Аввалин бор иҷро мекунем
      callbackRef.current(...args);
      lastRunRef.current = now;
    } else {
      // Тоза кардани timeout-и қаблӣ
      if (timeoutRef.current) {
        clearTimeout(timeoutRef.current);
      }
...
export const useThrottledValue = <T>(value: T, limit: number = 500): T => {
  ...
  useEffect(() => {
    ...
    if (timeSinceLastUpdate >= limit) {
      // Фавран навсозӣ мекунем
      setThrottledValue(value);
      lastUpdatedRef.current = now;
    } else {
      // Timeout барои навсозии баъдӣ
      if (timeoutRef.current) {
        clearTimeout(timeoutRef.current);
      }
